...
class VendorOrderApiTest extends TestCase
{
    ...
    public function test_vendor_ship_generates_otp_and_confirm_delivery_completes_order(): void
    {
        ...
        $ship->assertOk()
            ...
            ->assertJsonPath('data.order.delivery_otp.single_use', true)
            ->assertJsonPath('data.order.delivery_otp.expires', false)
            ->assertJsonPath('data.order.actions.can_resend_delivery_otp', true);

        $mapping->refresh();
        $this->assertNotEmpty($mapping->delivery_otp);
        $this->assertSame('in_app', $mapping->delivery_otp_sent_to);
        $this->assertNull($mapping->delivery_otp_expires_at);
    ...
    public function test_vendor_delivery_otp_resend_replaces_code_and_is_single_use(): void
    {
        ...
        $this->assertNotEmpty($firstOtp);
        $this->assertNull($mapping->delivery_otp_expires_at);

        // No cooldown — resend works immediately and replaces the previous code.
        $this->withToken($token)->postJson('/api/vendor/orders/'.$mapping->id.'/resend-delivery-otp')
            ...
            ->assertJsonPath('data.order.delivery_otp.expires', false)
            ->assertJsonPath('data.order.actions.can_resend_delivery_otp', true)
            ->assertJsonPath('data.order.actions.resend_delivery_otp_available_in_seconds', 0);

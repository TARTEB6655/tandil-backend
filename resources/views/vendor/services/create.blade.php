<x-vendor-layout>
    <x-dashboard.page-header title="Add Service" subtitle="Create a service for your catalog." />
    <form method="POST" action="{{ route('vendor.services.store') }}" enctype="multipart/form-data" class="max-w-xl space-y-4 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        @csrf
        <div>
        ...
        <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">Save Service</button>
    </form>
</x-vendor-layout>

<x-supervisor-layout>
    ...
    <div class="mb-4 flex items-center justify-between">
        <a href="{{ route('supervisor.notifications.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800">&larr; Back to notifications</a>
        <form action="{{ route('supervisor.notifications.destroy', $notification->id) }}" method="POST">
            @csrf
            @method('DELETE')

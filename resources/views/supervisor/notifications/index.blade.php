<x-supervisor-layout>
    ...
    <div class="mb-4 sm:mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
            <div>
                ...
                <p class="mt-1 text-xs sm:text-sm text-gray-500">Stay updated with visits, reports, and complaints in your areas.</p>
            </div>
            @if($unreadCount > 0)
                <form action="{{ route('supervisor.notifications.mark-all-read') }}" method="POST" class="inline">
                    @csrf
    ...
    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="inline-flex rounded-lg border border-gray-200 bg-white p-1">
            <a href="{{ route('supervisor.notifications.index', ['filter' => 'all', 'q' => request('q')]) }}" class="px-3 py-1.5 text-xs sm:text-sm rounded-md {{ $activeFilter === 'all' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100' }}">All</a>
            <a href="{{ route('supervisor.notifications.index', ['filter' => 'unread', 'q' => request('q')]) }}" class="px-3 py-1.5 text-xs sm:text-sm rounded-md {{ $activeFilter === 'unread' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100' }}">Unread</a>
            <a href="{{ route('supervisor.notifications.index', ['filter' => 'read', 'q' => request('q')]) }}" class="px-3 py-1.5 text-xs sm:text-sm rounded-md {{ $activeFilter === 'read' ? 'bg-gray-900 text-white' : 'text-gray-600 hover:bg-gray-100' }}">Read</a>
        </div>
        <form method="GET" action="{{ route('supervisor.notifications.index') }}" class="flex items-center gap-2">

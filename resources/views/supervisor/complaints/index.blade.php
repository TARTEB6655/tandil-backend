<x-supervisor-layout>
    ...
    <div class="mb-4 sm:mb-6">
        <h1 class="text-lg sm:text-xl font-medium text-gray-900">Complaints</h1>
        <p class="mt-1 text-xs sm:text-sm text-gray-500">Manage complaints related to visits in your supervised areas.</p>
    </div>

    <!-- Search Bar -->
    <div class="mb-4 sm:mb-6">
        <form action="{{ route('supervisor.complaints.index') }}" method="GET" class="w-full">
            <div class="relative">
                ...
                @if($search ?? '')
                    <button type="button"
                            onclick="window.location.href='{{ route('supervisor.complaints.index') }}'"
                    ...
                    @forelse($complaints as $complaint)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500 hidden sm:table-cell">
                                {{ $complaint->visit && $complaint->visit->subscription && $complaint->visit->subscription->client ? $complaint->visit->subscription->client->name : 'N/A' }}
                            </td>

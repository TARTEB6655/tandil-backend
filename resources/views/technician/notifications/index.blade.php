<x-technician-layout>
    <div class="space-y-5 sm:space-y-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            @if($unreadCount > 0)
                <form method="POST" action="{{ route('technician.notifications.mark-all-read') }}" class="inline shrink-0">
                    @csrf
        ...
        <x-notification-inbox-toolbar route-name="technician.notifications.index" :show-audience-filter="false" />
        ...
        <div class="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/90 dark:bg-slate-900/40 px-4 py-3 sm:px-5 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3 sm:gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                <form method="POST" action="{{ route('technician.notifications.destroy-all') }}" class="inline" onsubmit="return confirm('Delete all notifications?');">
                    @csrf
                    @foreach(request()->query() as $key => $value)
                        @if(is_string($value))
                            <input type="hidden" name="{{ $key }}" value="{{ $value }}" />
        ...
        <form method="POST" action="{{ route('technician.notifications.destroy-bulk') }}" id="form-notifications-bulk">
            @csrf
            <x-notification-inbox-rows :notifications="$notifications" route-name="technician.notifications" />
        </form>

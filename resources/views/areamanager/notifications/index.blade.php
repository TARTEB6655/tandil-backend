<x-areamanager-layout>
    <div class="space-y-5 sm:space-y-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            @if($unreadCount > 0)
                <form method="POST" action="{{ route('areamanager.notifications.mark-all-read') }}" class="inline shrink-0">
                    @csrf
        ...
        <x-notification-inbox-bulk-actions
            :destroy-all-route="route('areamanager.notifications.destroy-all')"
        ...
        <form method="POST" action="{{ route('areamanager.notifications.destroy-bulk') }}" id="form-notifications-bulk">
            @csrf
            <x-notification-inbox-rows :notifications="$notifications" route-name="areamanager.notifications" />
        </form>

...
<x-admin-layout>
    <div class="space-y-6 max-w-2xl">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Language & Region</h1>
                <p class="mt-1 text-sm text-gray-500">Change app language</p>
            </div>
        </div>
        @if(session('success'))
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <form method="POST" action="{{ route('admin.settings.language.store') }}" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Language</label>
                    <select name="language" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                ...
                <div>
                    ...
                    <input type="text" name="region" value="{{ $region }}" placeholder="e.g. US, SA" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" maxlength="10">
                </div>

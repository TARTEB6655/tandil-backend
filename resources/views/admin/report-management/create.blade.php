<x-admin-layout>
    <div class="space-y-6">
        ...
        <div class="bg-white shadow rounded-lg p-6 max-w-2xl">
            <form method="POST" action="{{ route('admin.report-management.store') }}">
                @csrf
                <div class="space-y-4">
                    <div>
                        <label for="title" class="block text-sm font-medium text-gray-700">Title <span class="text-red-500">*</span></label>
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" placeholder="e.g. Monthly Financial Report">
                    </div>
                    <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="parameters[start_date]" id="start_date" value="{{ old('parameters.start_date') }}"
                                   class="mt-1 block w-full rounded-md border-gray-300">
                        </div>
                        <div>
                            <label for="end_date" class="block text-sm font-medium text-gray-700">End date</label>
                            <input type="date" name="parameters[end_date]" id="end_date" value="{{ old('parameters.end_date') }}"
                                   class="mt-1 block w-full rounded-md border-gray-300">
                        </div>
                    </div>
                </div>

<x-admin-layout>
    ...
    <div class="mx-auto max-w-3xl space-y-5">
        <div class="flex items-center justify-between gap-3">
            ...
            <a href="{{ route('admin.loyalty.campaigns.create') }}" class="ly-btn inline-flex items-center rounded-lg px-3 py-2 text-sm font-semibold">+</a>
        </div>
        ...
        <div class="rounded-2xl ly-bg-beige p-5">
            <div class="flex items-start justify-between gap-3">
                <div>
                    <p class="text-base font-semibold text-gray-900">Launch campaigns that boost repeat orders.</p>
                    <p class="mt-1 text-sm text-gray-600">Schedule multipliers, target cities, and keep seasonal offers easy to manage.</p>
                </div>
                <a href="{{ route('admin.loyalty.campaigns.create') }}" class="ly-btn shrink-0 rounded-lg px-3 py-2 text-sm font-semibold">+ New campaign</a>
            </div>
            <div class="mt-4 grid grid-cols-3 gap-2 rounded-xl bg-white px-3 py-3 text-center text-sm">
                ...
                <div><p class="font-bold ly-green">{{ $summary['live'] }}</p><p class="text-xs text-gray-500">Live</p></div>
                <div><p class="font-bold ly-green">{{ $summary['top_boost'] }}</p><p class="text-xs text-gray-500">Top boost</p></div>
            </div>
        </div>
        ...
        <div class="space-y-3">
                ...
                <div class="rounded-2xl ly-bg-beige p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">{{ $campaign['cities'] }} · {{ $campaign['customer_targeting'] }}</p>
                        </div>
                    ...
                    <div class="mt-4 flex items-center justify-between border-t border-black/5 pt-3">
                        ...
                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.loyalty.campaigns.edit', $campaign['id']) }}" class="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-[#1B4332]/30 text-[#1B4332] hover:bg-white">
            ...
            @empty
                <div class="rounded-2xl border border-dashed border-gray-300 bg-white p-8 text-center text-sm text-gray-500">No campaigns yet.</div>
            @endforelse
        </div>
    </div>
</x-admin-layout>

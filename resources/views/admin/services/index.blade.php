    ...
    <div class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Click a service card to see the products linked to it.
                </p>
            </div>
            <div class="flex items-center gap-3 flex-wrap">
                <a href="{{ route('admin.services.create') }}"
                   class="inline-flex items-center gap-2 px-4 py-2.5 bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            @forelse($services as $svc)
                @php
                    $icon = $svc->icon && isset($iconMap[$svc->icon]) ? $iconMap[$svc->icon] : '📋';
                    $gradient = $gradients[$svc->icon ?? ''] ?? 'from-gray-500/10 to-gray-500/10 dark:from-gray-500/20 dark:to-gray-500/20 border-gray-200/50 dark:border-gray-500/30';
                @endphp
                <a href="{{ route('admin.services.category', $svc->id) }}"
                    ...
                    <div class="p-6 flex flex-col h-full min-h-[180px]">
                        <div class="flex items-start justify-between gap-3 mb-3">
                            ...
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
                                {{ $svc->products_count }} {{ Str::plural('product', $svc->products_count) }}
                            </span>
                        </div>
                        ...
                        @if($svc->category)
                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Category: {{ $svc->category->name }}</p>
                        @endif
                        @if($svc->description)
            ...
            @empty
                <div class="col-span-full rounded-2xl border border-dashed border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800/50 p-12 text-center">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Create your first service to get started.</p>
                    <a href="{{ route('admin.services.create') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-teal-600 hover:bg-teal-700 text-white text-sm font-medium rounded-lg shadow-sm">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
                        Create Service
                    </a>
                </div>

<x-admin-layout>
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Add video banner</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Featured video for the customer app home screen. Max 30MB — auto-compressed for speed.</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
            <form method="POST" action="{{ route('admin.video-banners.store') }}" enctype="multipart/form-data" class="p-6 space-y-8" id="video-banner-form">
                @csrf

                <div class="space-y-4">
                    <h2 class="text-sm font-semibold text-gray-900 dark:text-gray-100 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700 pb-2">Video</h2>
                    <div>
                        ...
                        <input type="file" name="video" id="video" accept="video/mp4,video/quicktime,video/webm,video/ogg,video/x-m4v,.mp4,.mov,.webm,.ogg,.m4v" required
                        ...
                        <p class="mt-1.5 text-xs text-gray-500 dark:text-gray-400">MP4 recommended. Maximum <strong>30MB</strong>. Larger files are rejected.</p>
                        @error('video')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="e.g. See Tandil in action"
                    ...
                    <div>
                        ...
                        <input type="text" name="badge_text" id="badge_text" value="{{ old('badge_text') }}" placeholder="e.g. Watch now"
                    ...
                    <div>
                        ...
                        <input type="text" name="button_text" id="button_text" value="{{ old('button_text') }}" placeholder="e.g. Explore services"
                               class="w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 max-w-md">
                        @error('button_text')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                        @enderror
    ...
    <script>
        ...
        document.getElementById('video-banner-form').addEventListener('submit', function () {
            var btn = document.getElementById('submit-btn');
            btn.disabled = true;
            btn.textContent = 'Uploading & compressing…';
        });
    </script>
</x-admin-layout>

<div class="mb-4 sm:mb-6 flex items-start justify-between gap-3">
    ...
    <a href="{{ route($routePrefix . '.help-support.index') }}" class="text-sm text-indigo-600 hover:text-indigo-800">Back to Help & Support</a>
</div>
...
@if(!in_array($ticket->status, ['resolved', 'closed'], true))
    <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6">
        ...
        <form action="{{ route($routePrefix . '.help-support.reply', $ticket->id) }}" method="POST" class="space-y-3 max-w-xl">
            @csrf
            <div>
